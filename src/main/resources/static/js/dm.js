...
//리스닝 상태 만들기(구독 상태 만들기)
function subscribe(idm) {
    Object.keys(stomp.subscriptions).forEach(function(item) {//배열이기때문에 사용 가능
...
function onMessage(msg) {
    ...
    dmMsgContainerElem.scrollTop = dmMsgContainerElem.scrollHeight;//글 쓸때마다 스크롤 내려줌
}
...
function makeDmUserList(userList) {
    ...
    if(isNonExistent) {
        myFetch.get('/dm/reg', myJson => {
            ...
            subscribe(myJson.idm);//리스닝 상태 만들기
        }, {
            toiuser: oppoiuser
        });
    }
...
let isFirst = true;//처음에만 제일 밑으로 스크롤

function getDmMsgList(idm) {
    isFirst = false;
    if(msgParam.isNoMore) { return; }//없으면 통신 더이상 안하도록
    if(idm) {
        msgParam.idm = idm;
    }
    msgParam.page++;
...
function makeDmMsgList(msgList) {
    ...
    if(isFirst) {
        dmMsgContainerElem.scrollTop = dmMsgContainerElem.scrollHeight;//스크롤 가장 아래로
    } else {
